...
if ( ! class_exists( 'Bloghash_Admin' ) ) :

	/**
	 * Admin Class
	 */
	class Bloghash_Admin {
		...
		public function load_assets( $hook ) {
			...
			$texts = array(
				...
				'import_complete_desc'  => esc_html__( 'The demo has been imported.', 'bloghash' ) . ' <a href="' . esc_url( get_home_url() ) . '">' . esc_html__( 'Visit site.', 'bloghash' ) . '</a>',
			);
		...
		public function filter_update_footer( $text ) {
			...
			if ( bloghash_is_admin_page( $base ) ) {
				return apply_filters( 'bloghash_footer_version', esc_html__( 'BlogHash Theme', 'bloghash' ) . ' ' . BLOGHASH_THEME_VERSION . '<br/><a href="' . esc_url( 'https://twitter.com/peregrine-themes' ) . '" target="_blank" rel="noopener noreferrer"><span class="dashicons dashicons-twitter"></span></a><a href="' . esc_url( 'https://facebook.com/peregrine-themes' ) . '" target="_blank" rel="noopener noreferrer"><span class="dashicons dashicons-facebook"></span></a>' );
			} else {
				return $text;
			}
		}
		...
		public function admin_header() {
			...
			<div id="hester-header">
				<div class="hester-container">
					...
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=bloghash-dashboard' ) ); ?>" class="hester-logo">
						<img src="<?php echo esc_url( BLOGHASH_THEME_URI . '/assets/images/bloghash-logo.svg' ); ?>" alt="<?php echo esc_html( 'Bloghash' ); ?>" />
					</a>

					<span class="hester-header-action">
						<a href="<?php echo esc_url( admin_url( 'customize.php' ) ); ?>"><?php esc_html_e( 'Customize', 'bloghash' ); ?></a>
						<a href="<?php echo esc_url( 'http://docs.peregrine-themes.com/' ); ?>" target="_blank" rel="noopener noreferrer"><?php esc_html_e( 'Help Articles', 'bloghash' ); ?></a>
					</span>
		...
		public function admin_footer() {
			...
			if ( ! bloghash_is_admin_page( $base ) || bloghash_is_admin_page( $base, 'hester_wizard' ) ) {
				return;
			}
			?>
			<div id="hester-footer">
			<ul>
				<li><a href="<?php echo esc_url( 'http://docs.peregrine-themes.com/' ); ?>" target="_blank" rel="noopener noreferrer"><span><?php esc_html_e( 'Help Articles', 'bloghash' ); ?></span></span></a></li>
				<li><a href="<?php echo esc_url( 'https://www.facebook.com/groups/peregrine-themes/' ); ?>" target="_blank" rel="noopener noreferrer"><span><?php esc_html_e( 'Join Facebook Group', 'bloghash' ); ?></span></span></a></li>
				<li><a href="<?php echo esc_url( 'https://wordpress.org/support/theme/bloghash/reviews/#new-post' ); ?>" target="_blank" rel="noopener noreferrer"><span class="dashicons dashicons-heart" aria-hidden="true"></span><span><?php esc_html_e( 'Leave a Review', 'bloghash' ); ?></span></a></li>
			</ul>
		...
		public function admin_notices() {
			...
			// Display on Dashboard, Themes and Bloghash admin pages.
			if ( ! in_array( $screen->base, array( 'dashboard', 'themes' ), true ) && ! bloghash_is_admin_page() ) {
				return;
			}

			// Display if not dismissed and not on Bloghash plugins page.
			if ( ! bloghash_is_notice_dismissed( 'bloghash_notice_recommended-plugins' ) && ! bloghash_is_admin_page( false, 'bloghash-plugins' ) ) {

				$plugins = bloghash_plugin_utilities()->get_recommended_plugins();
				$plugins = bloghash_plugin_utilities()->get_deactivated_plugins( $plugins );
				...
				if ( is_array( $plugins ) && ! empty( $plugins ) ) {
					...
					$message = sprintf( wp_kses( __( 'BlogHash theme recommends the following plugins: %1$s.', 'bloghash' ), bloghash_get_allowed_html_tags() ), $plugin_list );

					$navigation_items = bloghash_dashboard()->get_navigation_items();

					bloghash_print_notice(
						array(
							'type'        => 'info',
							'message'     => $message,
							...
							'action_text' => esc_html__( 'Install Now', 'bloghash' ),
						)
					);
				}
			}
